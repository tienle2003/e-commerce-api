...
const sendMail = (receiver, mailBody, subject) => {
...
const sendVerifyEmail = (token, receiver) => {
  const mailBody = {
    body: {
  ...
  sendMail(receiver, mailBody, "VERIFY EMAIL FOR REGISTER ACCOUNT");
};

const sendResetPassword = (token, receiver) => {
  const mailBody = {
    body: {
      title: `HI ${receiver.name.toUpperCase()},`,
      intro: [
  ...
  sendMail(receiver, mailBody, "RESET PASSWORD");
};
